...
- 本项目当前 Human 已明确授权：后续任务经主 AI 独立验收为 PASS 后，主 AI 可直接将任务分支快进合并到 `main`，在合并后的 `main` 上复跑关键验证，并继续派发下一个任务；不必为每次合并重复询问。若验证失败或出现范围外问题，停止合并并报告。
- 远端 push 仍需遵循当前 Human 的明确授权范围；仅获得“直接合并”授权时，不自动推送，除非同一授权明确包含推送。
