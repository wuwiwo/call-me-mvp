...
// CM004_CHROME > CHROME_PATH > 常见安装位置 > which
const CHROME = resolveChrome(process.env.CM004_CHROME);
...

{
    ...
    check('legacy 配置读取无未捕获异常', pageErrors.length === 0, pageErrors.join(' | '));
    check(
    ...
    if (saved.ok) {
        ...
        check('迁移后图标不丢', b[0]?.icon === 'bell' && b[1]?.icon === 'heart', JSON.stringify(b));
    } else {
...

{
    ...
    if (saved.ok) {
        ...
        check('被编辑的按钮文案已更新', cs[0]?.message === '老自定义A-已改', JSON.stringify(cs[0]));
        check(
            '未编辑的自定义按钮 ID 完全不变',
...

{
    ...
    if (saved.ok) {
        const d = JSON.parse(saved.value);
        const cs = (d.parsed?.buttons || []).filter((x) => String(x.id).startsWith('custom_'));
...

{
    ...
    if (saved.ok) {
        ...
        const b = d.parsed?.buttons || [];
        const cs = b.filter((x) => String(x.id).startsWith('custom_'));
        check('删除后剩余 2 个自定义按钮', cs.length === 2, JSON.stringify(cs.map((x) => x.id)));
        check('被删按钮已移除', !cs.some((x) => x.message === '待删除B'), JSON.stringify(cs));
        check(
            '存活按钮 ID 未被连带改变（A 与 C）',
            cs[0]?.id === 'custom_1700000000001' && cs[1]?.id === 'custom_1700000000003',
            JSON.stringify(cs.map((x) => x.id))
        );
        check(
            '默认按钮编辑后 ID 仍为规范 ID',
...

{
    ...
    check('连续 3 次保存 ID 序列完全稳定', stable, JSON.stringify(details));
}
