...
check(
    'CONFIG.password 有 defaultPassword',
    typeof s.cfgDefault === 'string' && s.cfgDefault !== '',
    JSON.stringify(s.cfgDefault)
);
check(
    'CONFIG.password 有 expiryDays',
    typeof s.cfgExpiryDays === 'number',
    String(s.cfgExpiryDays)
);
...
check(
    '弹窗内 hint 已渲染',
    typeof s.hintText === 'string' && s.hintText.length > 0,
    JSON.stringify(s.hintText)
);
...
s = await snap();
check(
    `未超过 ${EXPIRY_DAYS} 天（第 ${EXPIRY_DAYS - 1} 天）→ 不弹窗`,
    s.hasModal === false,
    String(s.hasModal)
);
...
check(
    '错密码 → 报错文案显示',
    s.errorVisible === true && typeof s.errorText === 'string' && s.errorText.length > 0,
    JSON.stringify(s.errorText)
);
...
check(
    '默认密码 → 写入 passwordSetTime',
    !!s.timestamp && Number.isFinite(Number(s.timestamp)),
    String(s.timestamp)
);
...
check(
    'verify(默认密码) === true',
    semantics.verifyDefault === true,
    String(semantics.verifyDefault)
);
check('verify(错密码) === false', semantics.verifyWrong === false, String(semantics.verifyWrong));
check(
    'setPassword 覆盖写入 accessPassword',
    semantics.storedAfterSet === 'my-new-pw',
    String(semantics.storedAfterSet)
);
check(
    'setPassword 后 verify(新密码) === true',
    semantics.verifyNew === true,
    String(semantics.verifyNew)
);
check(
    'setPassword 后 verify(默认密码) === false',
    semantics.verifyDefaultAfterSet === false,
    String(semantics.verifyDefaultAfterSet)
);
check(
    'clearPassword 清掉 accessPassword',
    semantics.storedAfterClear === null,
    String(semantics.storedAfterClear)
);
check(
    'clearPassword 清掉 passwordSetTime',
    semantics.timeAfterClear === null,
    String(semantics.timeAfterClear)
);
check(
    'clearPassword 后回退默认密码',
    semantics.verifyDefaultAfterClear === true,
    String(semantics.verifyDefaultAfterClear)
);
...
for (const l of LANGS) {
    ...
    check(
        `${l}: hint 与翻译表一致`,
        hint === s.hints[l],
        `页面=${JSON.stringify(hint)} 表=${JSON.stringify(s.hints[l])}`
    );
}
check(
    '四语言 hint 取值互不相同',
    new Set(hintVals).size === LANGS.length,
    JSON.stringify(hintVals.map((h) => h.slice(0, 24)))
);
