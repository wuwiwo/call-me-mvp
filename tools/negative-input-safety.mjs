...
const INJECTION_MARKERS = ['元素注入', '事件属性注入', '未执行', '字面文本', '注入片段'];
const ALLOWLIST_MARKERS = ['白名单外', '回退为允许列表内的图标', '原值被原样保留'];
...
log(`修复版本汇总   : ${baseline ? baseline.summary : '(未执行)'} (预期 0 failed)`);
log(`回退版本退出码 : ${reverted ? reverted.code : '(未执行)'} (预期非 0)`);
log(`回退版本汇总   : ${reverted ? reverted.summary : '(未执行)'}`);
log(`回退版本注入类失败项     : ${injectionFails.length} 条 (预期 > 0)`);
injectionFails.slice(0, 8).forEach((f) => log(`     - ${f}`));
log(`回退版本允许列表类失败项 : ${allowlistFails.length} 条 (预期 > 0)`);
...
const revertedOk =
    reverted && reverted.code !== 0 && injectionFails.length > 0 && allowlistFails.length > 0;
